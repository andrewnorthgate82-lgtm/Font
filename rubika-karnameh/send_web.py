...
def make_recipients_xlsx(path: Path):
    ...
    try:
        ...
        g = wb.create_sheet("راهنما")
        g.sheet_view.rightToLeft = True
        lines = [
            ("📖 راهنمای فایل مخاطبین", True, 14),
            ("", False, 11),
            ("✔ ستون «نام در روبیکا» مهم‌ترین ستون است: نام هر مسئول را", False, 11),
            ("    همان‌طور که در لیست گفتگوهای روبیکای خودتان می‌بینید بنویسید؛", False, 11),
            ("    ارسال با جستجوی همین نام انجام می‌شود.", False, 11),
            ("✔ فقط کسانی که قبلاً با آن‌ها گفتگو داشته‌اید پیدا می‌شوند؛", False, 11),
            ("    برای بقیه یک بار از گوشی بهشان پیام بدهید تا گفتگو ساخته شود.", False, 11),
            ("✔ شماره موبایل را هم بنویسید (برای شناسایی و پشتیبانی).", False, 11),
            ("✔ نام ناحیه (ستون اول) باید دقیقاً مثل نام فایل تصویر باشد:", False, 11),
            ("    تصویر «کارنامه_ناحیه ۴.png» ← در اکسل بنویسید: ناحیه ۴", False, 11),
            ("✔ شماره را هر شکلی بنویسید (۰۹۱۲… یا +۹۸۹۱۲…)؛ خودش درست می‌خواند.", False, 11),
            ("✔ اگر یک نفر دو نقش دارد، همان یک شماره را دوبار بنویسید؛ یک پیام می‌گیرد.", False, 11),
            ("✔ ناحیه‌ای که نام یا شماره ندارد، رد می‌شود (خطا نمی‌دهد).", False, 11),
            ("", False, 11),
        ...
        for r, (txt, bold, size) in enumerate(lines, 1):
